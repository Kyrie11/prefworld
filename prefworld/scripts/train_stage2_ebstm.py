...
def main() -> None:
    ...
    parser.add_argument("--config", required=True, help="Path to stage2_eb.yaml")
    ...
    # Load stage-1 template + PC
    stage1_ckpt = cfg["train"].get("stage1_checkpoint")
    if stage1_ckpt:
    ...
    for epoch in range(int(tcfg.get("max_epochs", 10))):
        model.train()
        pbar = tqdm(loader, desc=f"Stage2-EB epoch {epoch}")
        ...
        for batch in pbar:
            ...
            out = model(
                batch,
                run_pc=True,   # needed to infer z for agents
                run_eb=True,
                detach_pc_for_eb=True,
                pc_query_ratio=0.0,  # inference-style posterior
                lambda_con=0.0,
                lambda_distill_mu=0.0,
                lambda_distill_cov=0.0,
                lambda_prior=0.0,
                lambda_overlap=0.0,
                lambda_mod=0.0,
                n_z_samples=1,
            ...
            loss = float(tcfg.get("w_eb", 1.0)) * out.losses[eb_loss_key]
            ...
            pbar.set_postfix(
                ...
                eb=float(out.losses.get("loss_eb", torch.tensor(0.0)).item()),
    ...
    print(f"Stage-2 done. Best checkpoint: {best_path}")
